...
const CONFIG = {
  ...
  BEE: {
    ID: process.env.BEE_CHANNEL,
    LIMIT: Number(process.env.BEE_REACTION_LIMIT),
    EMOJI: process.env.BEE_EMOJI,
  },
};
...
module.exports = async (reaction) => {
  ...
  if (emoji.name === CONFIG.SHAME.EMOJI && count === CONFIG.SHAME.LIMIT) {
    ...
    const alreadyOnBoard = fetch.some((m) =>
  ...
  if (
    (message.reactions.cache.find((r) => r.emoji.name === CONFIG.MPREG.EMOJI)
      ?.count ?? 0) === CONFIG.MPREG.LIMIT &&
    (message.reactions.cache.find((r) => r.emoji.name === CONFIG.CHAIR.EMOJI)
      ?.count ?? 0) === CONFIG.CHAIR.LIMIT
  ) {
  ...
  if (emoji.name === CONFIG.BEE.EMOJI && count === CONFIG.BEE.LIMIT) {
    const beeChannel = botClient.channels.cache.get(CONFIG.BEE.ID);
    if (!beeChannel) return; // cuz where the fuck would you put the pinned message anyway ?

    // so you see if the message is already on the board why the fuck would you pin it again ? Are you stupid ? What is your problem ?
    const fetch = await beeChannel.messages.fetch({ limit: 25 });
    const alreadyOnBeeoard = fetch.some((m) =>
      m.embeds[0]?.footer?.text?.endsWith(message.id),
    );

    if (!alreadyOnBeeoard) {
      const beeEmbed = new EmbedBuilder() //i used an embed builder online : https://embed.dan.onl/ its cool and it works well plz us that cuz its cool
        .setColor(0xfff317)
        .setAuthor({
          name: message.author.tag,
          iconURL: message.author.displayAvatarURL(),
